  ...
  <div class="flex flex-col p-4 bg-white border-t border-gray-200">
    <div class="flex items-center">
      ...
      <input v-model="message" @keyup.enter="sendMessage" placeholder="Type a message..."
        class="flex-1 px-4 py-3 mr-2 text-sm bg-gray-100 border-none rounded-lg focus:outline-none" type="text" />
      <div v-if="files.length > 0" class="flex mt-2 space-x-2 overflow-x-auto">
        <img v-for="file in files" :key="file.path" :src="file.fullUrl" class="w-16 h-16 object-cover rounded"
          alt="Uploaded image" />
      </div>
